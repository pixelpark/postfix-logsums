...
This module contains the underlaying parser class PostfixLogParser.
...
from . import DEFAULT_MAX_TRIM_LENGTH
from . import DEFAULT_SYSLOG_NAME
...
class PostfixLogParser(object):
    ...
    def __init__(
        ...
        verbose=0,
        day=None,
        syslog_name=DEFAULT_SYSLOG_NAME,
        zero_fill=False,
        detail_verbose_msg=False,
        detail_reject=True,
        detail_deferral=False,
        detail_bounce=False,
        detail_smtp=True,
        detail_smtpd_warning=True,
        ignore_case=False,
        rej_add_from=False,
        smtpd_stats=False,
        extended=False,
        no_no_message_size=False,
        verp_mung=None,
        compression=None,
    ...
    def parse_fh(self, fh, filename, compression=None):
        ...
        if compression == "gzip":
            LOG.debug(_("Reading {w} compressed file {f!r} ...").format(w="GZIP", f=filename))
            self.read_gzip(cdata)
            return True

        if compression == "bzip2":
            LOG.debug(_("Reading {w} compressed file {f!r} ...").format(w="BZIP2", f=filename))
            self.read_bzip2(cdata)
            return True

        if compression == "lzma":
            LOG.debug(_("Reading {w} compressed file {f!r} ...").format(w="LZMA", f=filename))
    ...
    def eval_line(self, line):
        ...
        if result:
            ...
            if self.verbose > 1:
                LOG.debug(
                    _("Did not found Postfix command and QID from: {}").format(self._cur_msg)
                )
            return
        if self.verbose > 3:
            LOG.debug(
                "Postfix command {cmd!r}, qid {qid!r}, message: {msg}".format(
    ...
    def _incr_smtpd_client_counters(self, client):
        ...
        if self.verbose > 2:
            msg = (
                "Increasing smtpd_client_counters for client {c!r}, domain {d!r} and " "qid {q!r}."
            ).format(c=client, d=domain, q=qid)
            LOG.debug(msg)
    ...
    def _eval_relayed_msg(self, addr, relay, delay, status, rest):
        ...
        if self.verbose > 2:
            msg = (
                ...
                "rest={rst!r}."
            ).format(a=addr, r=relay, d=delay, s=status, rst=rest)
            LOG.debug(msg)
